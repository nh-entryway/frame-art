...
export async function saveArtSubmission({ high, low, buffalo, scene, imageUrl, from, phone, timestamp }) {
  const payload = {
    mode: 'art',
    high,
    low,
  ...
  await put(`${ART_PREFIX}latest.json`, JSON.stringify(payload, null, 2), {
    access: 'public',
    addRandomSuffix: false,
    allowOverwrite: true,
    contentType: 'application/json',
...
export async function getLatestArt() {
  try {
    const { blobs } = await list({ prefix: `${ART_PREFIX}latest.json` });
    if (blobs.length === 0) return null;
